...
The histogram below shows the distribution of steps made in a day.  The mean value, `r trunc(mean(total_steps_by_date$total_steps, na.rm = TRUE))`, and median value, `r median(total_steps_by_date$total_steps, na.rm = TRUE)`, are represented by the green line

```{r}
library(ggplot2)

ggplot(data=total_steps_by_date, aes(x=total_steps)) + 
  geom_histogram(binwidth=diff(range(total_steps_by_date$total_steps, na.rm = TRUE))/10, fill="grey") +
  labs(title="Distribution of Daily Step Count in Human Activity Data") +
  xlab("Total steps per day") +
  ylab("Count") +
  geom_vline(xintercept = mean(total_steps_by_date$total_steps, na.rm = TRUE), colour="green")   
```

The precise values for mean and median numbers of steps taken per day are shown below
